...
#        self.log.info("from spanned process: navigating to  : ", self.urlbar.text())
...
#        logit("nous sommes à l'URL : "+self.urlbar.text())
#        logit("Now exiting")
...
def main(url, queue):
    logfile = open("D:/Users/Papa/Documents/GIT/cal-noosfere/webengine_plugin/log_spaned.txt", "a")
    txt=strftime("%H:%M:%S", gmtime())+' logfile is open now'
    logfile.write(txt+"\n")
    logfile.flush()
    txt=strftime("%H:%M:%S", gmtime())+' queue : '+type(queue)
    logfile.write(txt+"\n")
    logfile.flush()
    app = QApplication([])
    window = MainWindow()
